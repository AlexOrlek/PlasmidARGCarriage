...
taxonomy_replicon_freq <- finaldftrunc_frequency_tables %>% group_by(Species) %>% count(RepliconCarriage) %>% right_join((taxonomy_freq %>% rename('total plasmids' = 'n')), by = 'Species') %>% select(HostTaxonomy, Species, RepliconCarriage, n, `total plasmids`) %>% pivot_wider(names_from = RepliconCarriage, values_from = n) %>% mutate(across(where(is.integer), ~ ifelse(is.na(.x), 0, .x))) %>% mutate(typed = `single-replicon` + `multi-replicon`) %>% mutate(pct_typed = round((typed / `total plasmids`)*100, 1)) %>% mutate(typed = str_c(typed, ' (', pct_typed, ')')) %>% select(-pct_typed) %>% arrange(HostTaxonomy, desc(`total plasmids`), Species)
