pacman::p_load(ggplot2,ggpubr,cowplot,gridExtra)
...
p<-ggplot(tax_rep_contingency, aes(x=HostTaxonomy,y=Freq,fill=RepliconCarriage)) + geom_bar(position = 'dodge',stat='identity') + labs(x='Host taxonomy',y='Frequency') + theme(axis.text.x=element_text(angle=30,hjust=1),legend.position = 'top',plot.title = element_text(hjust=0.5,size=11)) + scale_x_discrete(limits=c("Enterobacteriaceae", "Proteobacteria_other", "Firmicutes","other")) + scale_fill_discrete(labels=c('untyped','single-replicon','multi-replicon')) + ggtitle('Replicon carriage:') + guides(title=NULL,fill=guide_legend(title=NULL,nrow=1))
