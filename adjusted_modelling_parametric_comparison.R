...
removedparametricvariables<-list(NA,c('integronbinary'),c('bacmetbinary'),c('reptypes'),c('taxa'),c('reptypes'),c('bacmetbinary'),c('integronbinary'),c('bacmetbinary','integronbinary'))
...
# unadjusteddf_edit<-unadjusteddf[!unadjusteddf$FactorVariable %in% removedvars,]
# 
# unique(unadjusteddf_edit$FactorVariable)
# 
# 
# adjusteddf_edit<-adjusteddf[!adjusteddf$FactorVariable %in% removedvars,]
# #}
...
# i<-1
# modelname<-alternativemodelnames[i]
# removedvar<-removedparametricvariables[i]
# print(modelname)
# #read adjusted output from alternative model
# adjusteddf_alternative<-read.table(gsub('%s',modelname,'output_adjusted/%s/adjustedodds.tsv'),as.is = TRUE,sep='\t',header=TRUE)
# #adjusteddf_alternative$logOddsRatio<-as.numeric(adjusteddf_alternative$logOddsRatio)  #!!!remove
# #re-order by factor level
# adjusteddf_alternative<-adjusteddf_alternative[order(adjusteddf_alternative$FactorVariable),]
# #edit unadjusted and adjusted mainmodel output to match rows of adjusted alternative (remove rows of removed factor)
# unadjusteddf_edit<-unadjusteddf
# adjusteddf_edit<-adjusteddf
# if (!is.na(removedparametricvariables[i])) {
#   unadjusteddf_edit<-unadjusteddf[unadjusteddf$FactorVariable!=removedvar,]
#   adjusteddf_edit<-adjusteddf[adjusteddf$FactorVariable!=removedvar,]
# }
# #append log-odds comparisons vs unadjusted and adjusted mainmodel
# stopifnot(all((adjusteddf_alternative$ResistanceClass==unadjusteddf_edit$ResistanceClass)==TRUE))
# stopifnot(all((adjusteddf_alternative$FactorVariable==unadjusteddf_edit$FactorVariable)==TRUE))
...
#!!!cut
#adjusteddf_alternative$logOddsRatio_unadjusted<-unadjusteddf_edit$logOddsRatio
#adjusteddf_alternative$logOddsRatio_difference_vs_unadjusted<-adjusteddf_alternative$logOddsRatio-unadjusteddf_edit$logOddsRatio
#adjusteddf_alternative$logOddsRatio_mainmodel<-adjusteddf_edit$logOddsRatio
#adjusteddf_alternative$logOddsRatio_difference_vs_mainmodel<-adjusteddf_alternative$logOddsRatio-adjusteddf_edit$logOddsRatio
...
# #read adjusted output from alternative model
# adjusteddf_alternative<-read.table(gsub('%s',modelname,'output_adjusted/%s/adjustedodds.tsv'),as.is = TRUE,sep='\t',header=TRUE)
# adjusteddf_alternative$logOddsRatio<-as.numeric(adjusteddf_alternative$logOddsRatio)
# #re-order by factor level
# adjusteddf_alternative<-adjusteddf_alternative[order(adjusteddf_alternative$FactorVariable),]
# #edit unadjusted and baseline adjusted output to match rows of adjusted alternative (remove rows of removed factor)
# unadjusteddf_edit<-unadjusteddf
# adjusteddf_edit<-adjusteddf
# if (!is.na(removedparametricvariables[i])) {
#   unadjusteddf_edit<-unadjusteddf[unadjusteddf$FactorVariable!=removedvar,]
...
# 
# 
# head(merge(adjusteddf_alternative,unadjusteddf_edit[,c('ResistanceClass','FactorVariable','FactorLevel','logOddsRatio')],by=c('ResistanceClass','FactorVariable','FactorLevel')))
# 
# 
# 
# 
# nrow(adjusteddf_alternative)
# nrow(unadjusteddf_edit)
# 
# 
# adjusteddf_edit[adjusteddf_edit$ResistanceClass=='tetracycline' & adjusteddf_edit$FactorVariable=='taxa','logOddsRatio']
# 
# adjusteddf_alternative$FactorLevel==adjusteddf_edit$FactorLevel
